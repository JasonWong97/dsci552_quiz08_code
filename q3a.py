...
dataset=pd.read_csv("data_to_show.csv")
...
for i in range(len(data_X)):
    if data_X[i]*3/2<data_y[i]:
        data1_X.append(data_X[i])
        data1_y.append(data_y[i])
    elif data_X[i]*3/2-150<data_y[i]:
        # if data_y[i]>150:
        #     continue
        data2_X.append(data_X[i])
        data2_y.append(data_y[i])
    else:
        data3_X.append(data_X[i])
        data3_y.append(data_y[i])
...
data_X_train = np.array(data_X[:-size_data]).reshape(-1,1)
data_X_test =  np.array(data_X[-size_data:]).reshape(-1,1)

# Split the targets into training/testing sets
data_y_train =  np.array(data_y[:-size_data]).reshape(-1,1)
data_y_test =  np.array(data_y[-size_data:]).reshape(-1,1)
